...
def forced_response(assembly, excitation_dict, rpm_linspace):
    ...
    Args:
      ...
      rpm_linspace: List of rpm values for which the analysis will be run for.
                Generated by numpy.linspace
    ...
    for rpm in rpm_linspace:
        # omegas, amplitudes = get_windmill_excitation(rpm)
        multipliers = list(excitation_dict)[0][:, 1] #check this TODO # because #TODO: connect multiplier lists -> if does not exists = None
        U = SystemExcitation(assembly.dofs, 2 * np.pi * np.array(multipliers) * rpm)

        for excitation_node, amplitude_percentage_list in excitation_dict:
            U.add_harmonic(excitation_node, amplitude_percentage_list[:, 0]*generator_torque(rpm)) # The rpm-torque profile has to be defined by the analysis #check this TODO
...
if __name__ == "__main__":
    """
    Create a model of a windmill as a system of three lumped masses connected by two shafts.
    Then run forced response analysis.
    """
    ...
    forced_response(assembly, rpm_linspace)